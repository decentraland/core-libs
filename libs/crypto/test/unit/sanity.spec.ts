...
describe('Authenticator sanity', () => {
  ...
  describe('when validating a production auth chain', () => {
    let chain: AuthChain
    let entityId: string

    beforeEach(() => {
      jest.useFakeTimers({ doNotFake: ['performance'] }).setSystemTime(0)
      entityId = 'QmfQtX7BrXzZx7UwupsqNwKGirSZUVBpjN5whujwcp9qyk'
      chain = [
        {
    ...
    it('should return a validation result equal to { ok: true, message: undefined }', async () => {
...
describe('Static signatures', () => {
  ...
  describe('when signing a message with a known real account', () => {
    let message: string
    ...
    beforeEach(() => {
      message = 'menduz'
      signature = Authenticator.createSignature(realAccount, message)
      recoveredAddress = recoverAddressFromEthSignature(signature, message)
    })

    it('should recover the real account address from the signature', () => {
      expect(recoveredAddress).toEqual(realAccount.address)
    })
  })
  ...
  describe('when creating an auth chain with a mock signature', () => {
    ...
    beforeEach(() => {
      chain = Authenticator.createAuthChain(realAccount, ephemeralIdentity, 10, 'test')
    })

    it('should produce three links', () => {
    ...
    it('should set the third link as ECDSA_SIGNED_ENTITY signed by the ephemeral account', () => {
