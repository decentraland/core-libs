...
describe('when validating entity structure', () => {
  ...
  describe('and the entity pointers is undefined', () => {
    beforeEach(async () => {
      deployment = buildDeployment({ entity: buildEntity({ pointers: undefined as unknown as string[] }) })
      result = await entityStructureValidationFn(deployment)
    })
  ...
  describe('and the entity pointers is null', () => {
    beforeEach(async () => {
      deployment = buildDeployment({ entity: buildEntity({ pointers: null as unknown as string[] }) })
      result = await entityStructureValidationFn(deployment)
    })
