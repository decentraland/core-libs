import type { EntityType } from '@dcl/schemas'
import {
  ADR_173_TIMESTAMP,
  ADR_232_TIMESTAMP,
  ADR_236_TIMESTAMP,
...
import type { DeploymentToValidate, ValidateFn } from '../types'
...
export function validateIfTypeMatches(entityType: EntityType, validate: ValidateFn): ValidateFn {
    ...
    (deployment) =>
      ...
      (deployment.entity.timestamp >= ADR_290_OPTIONAL_TIMESTAMP &&
        ...
        (deployment.entity.content?.length > 0 ||
          ...
          deployment.entity.metadata?.avatars?.some(
            (a: { avatar?: { snapshots?: unknown } }) => a?.avatar?.snapshots
          ))),
    validate
  )
}
