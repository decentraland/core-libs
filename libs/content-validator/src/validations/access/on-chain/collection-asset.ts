...
import type { BlockchainCollectionV1Asset, BlockchainCollectionV2Asset } from '@dcl/urn-resolver'
import { L1_NETWORKS, L2_NETWORKS, OK, validationFailed } from '../../../types'
import type {
  ...
  return async function validateFn(
    ...
    if (L1_NETWORKS.includes(network)) {
      ...
      const validateWearable = async (hashes: string[], block: number) => {
        try {
          return await L2.checker.validateWearables(ethAddress, contractAddress, asset.id, hashes, block)
        } catch (err) {
