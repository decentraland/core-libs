...
The list doubles as the switch deliberately: there is no way to accept the legacy payload without naming the fields that make doing so safe. Declaring it empty throws rather than being treated as disabled.

**Scope of the guarantee.** It covers the fields you list. An unlisted field stays unbound on the legacy path — which is sound only because a field the service does not authorize on cannot change an authorization decision. Derive the list from what the code actually reads.
