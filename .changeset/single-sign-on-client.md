...
Add `@dcl/single-sign-on-client` to the `core-libs` monorepo as a `localStorage`-backed identity store, dropping the hidden SSO iframe entirely. This redesign supersedes the previously published `2.0.0` and releases as the next major (`3.0.0`).
